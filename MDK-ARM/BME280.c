...
//------------------------------------------------
void BME280_ReadReg_S16(uint8_t Reg, int16_t *Value)//*declares a pointer(Value) to 16-bit signed int, (read this from right to left) Value is a pointer to 16-bit signed int;
{
  I2Cx_ReadData16(BME280_ADDRESS,Reg, (uint16_t*) Value);//converts Value of type(int16_t) to the (uint16_t) type;
}
//------------------------------------------------
...
{
  I2Cx_ReadData24(BME280_ADDRESS,Reg,Value);
  *(uint32_t *) Value = be24toword(*(uint32_t *) Value) & 0x00FFFFFF;// conventiruem value registra v BigEndian(be24toword) i tolko potom chitaem
}
//------------------------------------------------
...
//------------------------------------------------
void BME280_SetStandby(uint8_t tsb) {//datchik perehodit v standby, vo vremya standby my pri neobhodimosti chityvaem dannie
  uint8_t reg;
  reg = BME280_ReadReg(BME280_REG_CONFIG) & ~BME280_STBY_MSK;//registr configa clearim dlya standby moda, inymi slovami clearim kazhdiy bit v registre configa kotoriy HIGH v STBY_MSK
  reg |= tsb & BME280_STBY_MSK;//zadaem timing standby rezhima, setim timing registra standby 
  BME280_WriteReg(BME280_REG_CONFIG,reg);//zapuskaem standby, inicializiruem standby
}
//------------------------------------------------
...
//------------------------------------------------
void BME280_SetMode(uint8_t mode) {//dlya polnoi inicializacii datchika, nado vkluchit rezhim Normal v CTRL_MEAS registre
  uint8_t reg;
...
float BME280_ReadTemperature(void)
{
  float temper_float = 0.0f;
	...
	temper_raw >>= 4;//sdvigaem na 4 bita vpravo tak kak u nas hranitsya syroe(raw) znachenie temperatury, znachit i 4 LSBits ne uchastvuyut v pokazaniyah
	val1 = ((((temper_raw>>3) - ((int32_t)CalibData.dig_T1 <<1))) * //schitaem promezhutochnoe znachenie temperatury, s pomowiu formul v BME280 datasheete(4.2.3)
		((int32_t)CalibData.dig_T2)) >> 11;
	val2 = (((((temper_raw>>4) - ((int32_t)CalibData.dig_T1)) *
		((temper_raw>>4) - ((int32_t)CalibData.dig_T1))) >> 12) *
		((int32_t)CalibData.dig_T3)) >> 14;
	temper_int = val1 + val2;
	temper_float = ((temper_int * 5 + 128) >> 8);//output value "5123"
	temper_float /= 100.0f;//ex:chtoby pokazat znchenie v pravilnom resolutione (0.01degC) nado podelit 5123 na 100; 5123/100 = 51.23degC
...
float BME280_ReadPressure(void)
{
  float press_float = 0.0f;
	...
	pres_int = ((p >> 8) * 1000) + (((p & 0xff) * 390625) / 100000);// vrode by owybka zdes i v maine pri vyvode dannyh	
...
if(value !=BME280_ID)
	...
	while (BME280_ReadStatus() & BME280_STATUS_IM_UPDATE);// kogda 0-voi bit v registre Statusa stanovitsya 1 eto oznachaet chto datchik gotov k rabote posle restarta
